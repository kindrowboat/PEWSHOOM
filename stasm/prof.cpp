...
static Vec
GetBisector (const Vec &Prev, const Vec &This, const Vec &Next) // all in
{
static Vec p(2, ROWVEC), q(2, ROWVEC);  // static to reduce number of mallocs
...
static Vec PrevRow; PrevRow.dim(Shape.ncols(), ROWVEC); PrevRow = Shape.row(iPrev);
static Vec NextRow; NextRow.dim(Shape.ncols(), ROWVEC); NextRow = Shape.row(iNext);
...
static Vec          gProf(MAX_PROF_WIDTH_1D, MAX_PROF_WIDTH_1D);
...
static void
Get2dProfFromGradMats (Vec &Prof,                                // out
    unsigned ProfSpec, const Mat &Grads, const SHAPE &Shape,     // in
...
//-----------------------------------------------------------------------------
static void
InitGrads_GradBelowRight (Mat &Grads,           // out
                          const Mat &ImgMat)    // in
{
...
//-----------------------------------------------------------------------------
void
InitGradsIfNeeded (Mat &Grads,          // out
