...
{
//-----------------------------------------------------------------------------
Mat::Mat (const double Vals[], size_t nRows, size_t nCols)
{
setGslErrHandler();
...
Mat::Mat (const double Vals[], size_t nelems1, const char sIsRowVec[])
{
setGslErrHandler();
size_t iVal = 0;
...
void Mat::dim (size_t nRows, size_t nCols)
{
...
void Mat::dim (size_t nelems1, const char sIsRowVec[]) // redim, new contents unspecified
{
if (sIsRowVec)                      // create row vector if ANY string
    dim(1, nelems1);
else
...
void Mat::dimClear (size_t nRows, size_t nCols)
{
if (nRows == 0 || nCols == 0)     // zero size?
    {
    if (m)
...
void Mat::dimKeep (size_t nRows, size_t nCols)
{
if (nRows == 0 || nCols == 0) // zero size?
    {
    if (m)
...
if (nStartRow + nRows > A.nrows() || nStartCol + nCols > A.ncols())
    Err("Mat::reshape rows %d+%d > %d or columns %d+%d > %d",
        nStartRow, nRows, A.nrows(), nStartCol, nCols, A.ncols());

if (nRows * nCols < nNewRows * nNewCols)
    Err("Mat::reshape old dimensions %dx%d < new dimensions %dx%d",
        nRows, nCols, nNewRows, nNewCols);
#endif

Mat m1(nNewRows, nNewCols);
size_t i0 = 0, j0 = 0;
if (fRowMajor)
    {
...
Mat Mat::reshapeMe (size_t nNewRows, size_t nNewCols, bool fRowMajor,
                size_t nStartRow, size_t nStartCol, size_t nRows, size_t nCols)
{
Mat result(reshapeAux(*this, nNewRows, nNewCols,
...
Mat& Mat::operator= (const double Val)      // assign a scalar to all elements of mat
{
    for (size_t iRow = 0; iRow < nrows(); iRow++)
            for (size_t iCol = 0; iCol < ncols(); iCol++)
...
if (m->tda == ncols())
    {
    // faster version (TODO need to make the same change to all similar functions in this file)
...
Mat Mat::map (P_MAT_FUNC pFunc) const
{
Mat result(*this);
const size_t nRows = nrows();
for (size_t iRow = 0; iRow < nRows; iRow++)
...
Mat Mat::clearOffDiagEntries (void)
{
const size_t nRows = nrows();
for (size_t iRow = 0; iRow < nRows; iRow++)
    for (size_t iCol = 0; iCol < ncols(); iCol++)
...
bool Mat::fARowIsZero (double Val) const
{
const size_t nRows = nrows();
for (size_t iRow = 0; iRow < nRows; iRow++)
    {
...
//-----------------------------------------------------------------------------
bool Mat::fAllZeroes (double Val) const // true if all values zero, or equal to Val
{
bool fAllZero = true;
const size_t nRows = nrows();
for (size_t iRow = 0; iRow < nRows; iRow++)
...
// "optional mat tag string, can be read back or ignored by Mat::read"
...
void Mat::write (const char sFile[], FILE *pFile,
                const char sFormat[],
                const char sTag[],
...
char *Mat::sread (const char sFile[],
                  FILE *pFile,
                  char sOptionalTag[],
...
//-----------------------------------------------------------------------------
void Mat::read (const char sFile[],
                FILE *pFile,
                char sOptionalTag[],
                const char sExpectedTag[])
{
...
Vec SolveWithLU (const Mat &A, Vec &b)
{
#if GSL_RANGE_CHECK
...
//
// TODO Could be made more efficient by using arrays internally instead of mats.
...
//-----------------------------------------------------------------------------
MatView Mat::view (size_t iStartRow, size_t iStartCol,
...
const MatView Mat::view (size_t iStartRow, size_t iStartCol,
...
void Mat::fill (double Val, size_t iStartRow, size_t iStartCol,
...
Mat GetEigsForSymMat (const Mat &A, Vec &EigVals, bool fSort,
...
//TODO magic 1e5 chosen based for ASM model building, make it a parameter?
...
Mat Work(A); // gsl_eigen_symmv() destroys the A->m.data, so we use temp variable
...
Vec SolveWithCholesky (Mat &A, const Vec &b, bool *pfPosDef)
{
Vec x(A.nrows());
...
//-----------------------------------------------------------------------------
bool Mat::fIeeeNormal () const  // true if fIeeeNormal true for all elems of mat
{
const size_t nRows = nrows();
