...
// TODO following causes gcc 3.4.1 -Wextra warning: base class should be explicitly initialized
//MatView (const MatView &other) { init(other); }

// view matrix/view/array as a matrix by initializing a view with two dimensions

MatView (const Mat &other, size_t iStartRow, size_t iStartCol,
            size_t nRows, size_t nCols, int Tda=0) :Mat()
    {
...
MatView (double other[], size_t nRows, size_t nCols, int Tda) :Mat()
    {
...
MatView (double other[], size_t iStart, size_t nelems1, char sIsRowVec[]=NULL) :Mat()
    {
    sIsRowVec? init(other, iStart, 0, 1, nelems1, 1): init(other, 0, iStart, nelems1, 1);
    }
MatView (double other[], size_t nelems1, char sIsRowVec[]=NULL): Mat()
    {
...
void init (const Mat& other, size_t iStartRow, size_t iStartCol,
