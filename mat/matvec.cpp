...
while ((nWantedMats == 0 || iMat < nWantedMats) && !fgErr)
    ...
    if (s)
        ...
        // TODO revisit, this is really ugly!
...
// If not found, returned Mat.nrows() will be 0.
//
...
Mat FindMatInFile (const char sFile[],  // in
...
for (i = 0; i < MatV.size(); i++)
    {
    if (fMatchAttr(TagInts[iMat], Mask0, Mask1) && // filter first on attributes
